  ...
  <div class="pos-wrapper">
    ...
    <section class="products">
      ...
      <div class="custom-grid">
        ...
        <!-- BEAN -->
        <div :class="['custom-card', { 'card-done': selected.bean, 'card-locked': false }]"
          @click="openBean">
        ...
        <!-- BASE -->
        <div :class="['custom-card', { 'card-done': selected.base, 'card-locked': step < 2 }]"
          @click="openBase">
          <div class="card-icon">☕</div>
          <div class="card-content">
        ...
        <!-- MILK -->
        <div :class="['custom-card', { 'card-done': selected.milk, 'card-locked': step < 3 }]"
          @click="openMilk">
          <div class="card-icon">🥛</div>
          <div class="card-content">
            <h3>Sữa</h3>
    ...
    <div class="order-section">
      ...
      <div class="order-footer">
        ...
        <p class="total">
          Thành tiền: <strong>{{ finalPrice.toLocaleString() }}đ</strong>
          ...
          <div class="pay-customer-section">
            ...
            <div class="pay-customer-row">
              <div class="pay-field">
                ...
                <input v-model="payName" type="text" placeholder="Nhập tên khách..." class="pay-input" />
              </div>
              <div class="pay-field">
                <label>Số điện thoại</label>
                <input v-model="payPhone" type="text" inputmode="numeric"
                  placeholder="Nhập số điện thoại..." maxlength="10" class="pay-input" />
              </div>
            </div>
            <p class="pay-hint">Nếu bỏ trống, đơn sẽ hiển thị mã khách ẩn danh</p>
          </div>
      ...
      <div v-if="showReviewPopup" class="popup-overlay">
        <div class="review-popup">
          ...
          <div class="payment-footer">
            ...
            <button class="btn-confirm" @click="confirmPayment">✓ Xác nhận</button>
          </div>
        </div>
      </div>
          ...
          <div class="success-body">
            ...
            <template v-if="paymentMethod === 'cash'">
              <div class="success-receipt">
                ...
                <div class="success-row b"><span>Khách hàng</span><span>{{ payName || '(Ẩn danh)' }}</span></div>
                <div class="success-row b"><span>Số điện thoại</span><span>{{ payPhone || '—' }}</span></div>
                ...
                <div class="success-row b"><span>Mã GD</span><span class="tx-id">{{ txId }}</span></div>
                <div class="receipt-divider"></div>
                <!-- Sản phẩm -->
                <div v-for="(item, i) in orderedItems" :key="i" class="success-row b">
            ...
            <template v-else>
              <div class="success-receipt">
                ...
                <div class="success-row b"><span>Khách hàng</span><span>{{ payName || '(Ẩn danh)' }}</span></div>
                <div class="success-row b"><span>Thời gian</span><span>{{ txTime }}</span></div>
                <div class="success-row b"><span>Mã GD</span><span class="tx-id pink">{{ txId }}</span></div>
                <div class="receipt-divider"></div>
                <div class="success-row b"><span>Số tiền</span><span class="cash-need">{{ finalPrice.toLocaleString() }} VNĐ</span></div>
              </div>
            </template>
