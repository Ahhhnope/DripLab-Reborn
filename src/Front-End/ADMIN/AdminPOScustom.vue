...
async function addToCart() {
  ...
  const drinkItem = {
    ...
    beanName: selectedBean.value?.label ?? null,
    baseName: selectedBase.value?.label ?? null,
    ...
    toppingDetails: toppingOptions.value
      ...
      .map(t => ({ id: t.id, name: t.label, price: t.priceDelta })),
  }
              ...
              <div class="cup-wrap" :key="'cup-' + animTick">
                <svg class="cup-svg" viewBox="0 0 200 300" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                  ...
                  <g clip-path="url(#cupClip)">
                    <rect x="0" y="0" width="200" height="300" :fill="cupLayers.bean.show
                      ? 'rgba(30,12,4,0.08)'
                    ...
                    <g class="layers-split" :style="{ opacity: cupLayers.mixed.enabled ? 0 : 1 }">
                      <rect v-if="cupLayers.bean.show" class="liq-layer" x="0" :y="272 - cupLayers.bean.height * 2.44"
                        width="200" :height="cupLayers.bean.height * 2.44" :style="{
                          fill: cupLayers.bean.color,
                          '--delay': '0ms',
                        }" />
                      <rect v-if="cupLayers.milk.show" class="liq-layer" x="0" :y="272 -
                        (cupLayers.bean.height + cupLayers.milk.height) * 2.44
                        " width="200" :height="cupLayers.milk.height * 2.44" :style="{
                    ...
                    <g v-if="cupLayers.foam.show" class="foam-group">
                      ...
                      <g opacity="0.55" filter="url(#foamBlur)">
                        <circle v-for="i in 8" :key="'fb' + i" :cx="45 + i * 14" :cy="272 -
                          (cupLayers.bean.height +
                            (cupLayers.milk.show
                      ...
                      <g v-if="cupLayers.toppingsFx" class="topping-fx" :key="'fx-' + cupLayers.toppingsFx.fxKey">
                        <g v-if="cupLayers.toppingsFx.dustEnabled" class="fx-dust">
                          <circle v-for="i in 18" :key="'d' + i" :cx="40 + ((i * 9) % 120)" :cy="272 -
                            (cupLayers.bean.height +
                              cupLayers.milk.height +
                    ...
                    <ellipse v-if="cupLayers.bean.show && !cupLayers.mixed.enabled" :cx="100" :cy="272 -
                      (cupLayers.bean.height +
                        (cupLayers.milk.show ? cupLayers.milk.height : 0)) *
                      2.44
        ...
        <section class="step-panel">
          ...
          <div class="options-panel">
            ...
            <div v-if="currentStep === 2" class="options-section">
              ...
              <div class="options-grid options-grid--2">
                <button v-for="m in milkOptions" :key="m.id" class="option-card"
                  ...
                  <span class="option-card__swatch" :style="{
            ...
            <div v-if="currentStep === 3" class="options-section">
              ...
              <div class="options-grid options-grid--2">
                <button v-for="t in toppingOptions" :key="t.id" class="option-card option-card--check" :class="{
              ...
              <div class="level-group">
                ...
                <div class="level-pills">
                  <button v-for="opt in iceOptions" :key="opt" class="level-pill"
                    :class="{ 'level-pill--active': selection.ice === opt }" @click="selection.ice = opt">
                    {{ opt }}
                  </button>
                </div>
              </div>
              ...
              <div class="level-group">
                ...
                <div class="level-pills">
                  <button v-for="opt in sugarOptions" :key="opt" class="level-pill"
                    :class="{ 'level-pill--active': selection.sugar === opt }" @click="selection.sugar = opt">
                    {{ opt }}
                  </button>
                </div>
              </div>
