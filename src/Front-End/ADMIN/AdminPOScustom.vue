...
function onConfirmAndBack() {
  closeCartPopup()
  router.push('/QuanLyDonTaiQuay')
}
...
<template>
  <div class="brewing-page">
    <!-- Thêm nút quay lại ở đầu trang, trước <main> -->
    <div class="brewing-topbar">
      <button class="btn-go-back" @click="goBack">← Quay lại</button>
    </div>
        ...
        <aside class="cup-panel">
          <div class="cup-panel__inner">
                ...
                >
                  <defs>
                    ...
                    >
                      <stop
                        offset="0%"
                        :stop-color="cupLayers.bean.color || '#3d2010'"
                        stop-opacity="0.22"
                      />
                      <stop
                        offset="40%"
                    ...
                    <radialGradient id="foamGrad" cx="50%" cy="30%" r="60%">
                      <stop
                        offset="0%"
                        :stop-color="
                          cupLayers.milk.show ? '#fffcf5' : '#e8e0d8'
                        "
                      />
                      <stop
                        offset="60%"
                        :stop-color="
                          cupLayers.milk.show ? '#f3ead8' : '#d4ccc4'
                        "
                      />
                      <stop
                        offset="100%"
                        :stop-color="
                          cupLayers.milk.show ? '#e8d9c0' : '#c4bdb6'
                        "
                      />
                    </radialGradient>
                    <linearGradient
                  ...
                  <g clip-path="url(#cupClip)">
                    ...
                    <g
                      class="layers-split"
                      :style="{ opacity: cupLayers.mixed.enabled ? 0 : 1 }"
                    >
                      <rect
                      ...
                      <rect
                        ...
                        :y="
                          272 -
                          (cupLayers.bean.height + cupLayers.milk.height) * 2.44
                        "
                        width="200"
                    ...
                    <g
                      class="layers-mixed"
                      :style="{ opacity: cupLayers.mixed.enabled ? 1 : 0 }"
                    >
                      <rect
                    ...
                    <g v-if="cupLayers.foam.show" class="foam-group">
                      <ellipse
                        ...
                        :cy="
                          272 -
                          (cupLayers.bean.height +
                            (cupLayers.milk.show ? cupLayers.milk.height : 0)) *
                            2.44
                        "
                        rx="72"
                      ...
                      >
                        <g
                          v-if="cupLayers.toppingsFx.dustEnabled"
                          class="fx-dust"
                        >
                          <circle
                            v-for="i in 18"
                    ...
                    <g v-if="cupLayers.drizzle.enabled" class="drizzle">
                      ...
                      <path
                        d="M114 106 C122 121 109 131 119 148 S109 164 119 181"
                      />
                      <path
                        d="M140 98  C148 113 135 123 145 140 S135 156 145 173"
                      />
                    </g>
                    ...
                    <g v-if="cupLayers.bubbles.enabled" class="ice">
                      ...
                      <rect
                        ...
                        height="18"
                        rx="4"
                        :style="{ '--dur': '2.8s', '--delay-anim': '0.5s' }"
                        opacity="0.85"
                      />
                      <rect
                        x="120"
                    ...
                    <g v-if="cupLayers.bubbles.enabled" class="condensation">
                      ...
                      <ellipse
                        ...
                        rx="2.5"
                        ry="4"
                        :style="{ '--dur': '3.5s', '--delay-anim': '1.1s' }"
                      />
                      <ellipse
                        cx="44"
                        cy="202"
                  ...
                  />
                  <path
                    d="M 24 28 L 44 240 Q 43 258 60 266"
                    fill="none"
                  ...
                  />
                  <path
                    d="M 176 28 L 156 240 Q 157 258 142 266"
                    fill="none"
                  ...
                  <g
                    v-if="cupLayers.logo && cupLayers.logo.enabled"
                    class="cup-logo"
                  >
                    <image
                      :href="dripLabLogo"
                      :xlink:href="dripLabLogo"
                      x="56"
                      y="118"
                      width="88"
                      height="56"
            ...
            <div class="cup-summary">
              ...
              <div
                v-if="selectedMilk && selection.milk !== 'none'"
                class="cup-summary__row"
              >
                <span
                  class="cup-summary__dot"
        ...
        <section class="step-panel">
          ...
          <div class="stepper-grid">
            <button
              ...
              @click="goToStep(idx)"
            >
              <span class="stepper-cell__num">{{
                String(idx + 1).padStart(2, "0")
              }}</span>
              <span class="stepper-cell__label">{{ step.label }}</span>
              <span v-if="step.key === 'topping'" class="stepper-cell__count"
                >({{ toppingCountLabel }})</span
              >
              <span
                v-if="step.key === 'bean' && selectedBean"
                class="stepper-cell__value"
                >{{ selectedBean.label }}</span
              >
              <span
                v-if="step.key === 'base' && selectedBase"
                class="stepper-cell__value"
                >{{ selectedBase.label }}</span
              >
              <span
                v-if="step.key === 'milk' && selectedMilk"
                class="stepper-cell__value"
                >{{ selectedMilk.label }}</span
              >
              <span
                v-if="step.key === 'topping' && selection.toppings.size > 0"
                class="stepper-cell__value"
                >{{ selection.toppings.size }} đã chọn</span
              >
              <span
                v-if="step.key === 'size' && selectedSize"
                class="stepper-cell__value"
                >Size {{ selectedSize.label }}</span
              >
              <span
                v-if="step.key === 'confirm' && currentStep >= 5"
                class="stepper-cell__value"
                >Đá {{ selection.ice }} · Đường {{ selection.sugar }} ·
                {{ selection.quantity }} ly</span
              >
            </button>
          </div>
              ...
              <div class="options-grid options-grid--2">
                ...
                >
                  <span
                    class="option-card__swatch"
                    :style="{
                      background: m.color || 'rgba(0,0,0,0.06)',
                      border: m.color ? 'none' : '2px dashed #ccc',
                    }"
                  ></span>
                  <span class="option-card__name">{{ m.label }}</span>
                  <span class="option-card__sub">{{ m.sub }}</span>
                  <span class="option-card__price">{{
                    m.priceDelta ? "+" + formatVnd(m.priceDelta) : "Miễn phí"
                  }}</span>
                </button>
              </div>
              <div class="options-nav">
            ...
            <div v-if="currentStep === 3" class="options-section">
              ...
              <div class="options-grid options-grid--2">
                <button
                  ...
                  :class="{
                    'option-card--selected': selection.toppings.has(t.id),
                  }"
                  @click="toggleTopping(t.id)"
                >
                  <span class="option-card__checkbox">
                    <svg
                      v-if="selection.toppings.has(t.id)"
                      viewBox="0 0 14 14"
                      fill="none"
                    >
                      <path
            ...
            <div v-if="currentStep === 4" class="options-section">
              ...
              <div class="options-grid options-grid--3">
                <button
                  ...
                  :key="s.id"
                  class="option-card option-card--size"
                  :class="{ 'option-card--selected': selection.size === s.id }"
                  @click="selectSize(s.id)"
                >
                  <span class="option-card__size-badge">{{ s.label }}</span>
                  <span class="option-card__name">{{ s.sub }}</span>
                  <span class="option-card__price">{{
                    s.priceDelta ? "+" + formatVnd(s.priceDelta) : "Gốc"
                  }}</span>
                </button>
              </div>
              <div class="options-nav">
