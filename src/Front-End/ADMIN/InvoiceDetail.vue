                ...
                <div class="rounded-xl border border-stone-100 bg-stone-50/50 p-5">
                  ...
                  <div class="grid grid-cols-12 gap-y-2 gap-x-4">
                    ...
                    <div class="col-span-8 text-sm font-bold text-[#3C2A21]">
                      {{ invoice.isAnon ? "—" : (invoice.customer?.id || "—") }}
                    </div>

                    <div class="col-span-4 text-sm text-stone-400">Tên</div>
                    <div class="col-span-8 text-sm font-medium">
                      {{ invoice.isAnon ? "Khách vãng lai" : (invoice.receiverName || invoice.customer?.name || "—") }}
                    </div>

                    <div class="col-span-4 text-sm text-stone-400">SĐT</div>
                    <div class="col-span-8 text-sm font-medium">
                      <template v-if="invoice.isAnon">
                        Không có thông tin
                      </template>
                      <template v-else>
                    ...
                    <div class="col-span-4 text-sm text-stone-400" v-if="invoice.receive_type === 'Online'">Email</div>
                    <div class="col-span-8 text-sm font-medium" v-if="invoice.receive_type === 'Online'">
                      {{ invoice.receiverEmail || invoice.customer?.email || "—" }}
                    </div>

                    <div class="col-span-4 text-sm text-stone-400" v-if="invoice.receive_type === 'Online'">Địa chỉ</div>
                    <div class="col-span-8 text-sm font-medium" v-if="invoice.receive_type === 'Online'">
                ...
                <div class="space-y-3">
                  ...
                  >
                    ...
                    <div v-if="item.size" class="flex items-center gap-1.5">
                      <span class="text-[11px] font-semibold px-2 py-0.5 rounded bg-stone-100 text-stone-500 border border-stone-200">
                        Size {{ item.size }}
                      </span>
                      <span v-if="item.sizeExtra > 0" class="text-[11px] font-semibold text-amber-600">
                        +{{ item.sizeExtra.toLocaleString("vi-VN") }} đ
                      </span>
                    </div>

                    <div v-if="item.toppings?.length" class="flex flex-col gap-1">
                      <div v-for="(top, ti) in item.toppings" :key="ti" class="flex items-center gap-1.5">
                        <span
                        ...
                        <span v-if="top.price > 0" class="text-[11px] font-semibold text-amber-600">
  ...
  try {
    ...
    const itemsHTML = (inv.items || [])
      .map((item, idx) => {
        const toppingLines = (item.toppings || [])
          .map((t) => `<div class="sub-line">  + ${t.name}${t.price ? ` (+${fmt(t.price)}đ)` : ""}</div>`)
          .join("");
        ...
        const noteLine = item.note ? `<div class="sub-line italic">  * ${item.note}</div>` : "";
    ...
    <tr>
      <td class="item-col">
        ...
        ${sugarIceLine}${customLine}${toppingLines}${noteLine}
      </td>
    ...
    // Khách vãng lai → không in SĐT
    const customerBlock = inv.isAnon
      ? ""
      : `
    <tr>
      <td class="label">Khách hàng:</td>
      <td class="value">${inv.customer?.name || inv.receiverName || ""}</td>
    </tr>
    ${inv.customer?.phone && inv.customer.phone !== "-"
        ? `<tr><td class="label">SĐT:</td><td class="value">${inv.customer.phone}</td></tr>`
        : ""}`;
...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  body { background:#fff; font-family:'Courier New',Courier,monospace; font-size:13px; color:#111; width:302px; }
  #receipt { width:302px; padding:16px 14px 20px; background:#fff; }
  .logo-wrap { text-align:center; margin-bottom:6px; }
  .logo-img { max-width:130px; max-height:60px; object-fit:contain; }
  .logo-fallback { font-size:22px; font-weight:900; letter-spacing:6px; text-transform:uppercase; }
  .center { text-align:center; }
  .bold { font-weight:bold; }
  .sm { font-size:11px; }
  .italic { font-style:italic; }
  .dash { font-size:12px; color:#555; text-align:center; line-height:1.4; margin:4px 0; overflow:hidden; white-space:nowrap; }
  .receipt-title { text-align:center; font-size:15px; font-weight:bold; text-transform:uppercase; letter-spacing:2px; margin:6px 0 2px; }
  .invoice-id { text-align:center; font-size:12px; margin-bottom:4px; }
  ...
  .items-table th { font-weight:bold; text-align:left; padding:3px 0; font-size:11px; text-transform:uppercase; letter-spacing:1px; }
  ...
  .totals-table { width:100%; border-collapse:collapse; margin:4px 0; font-size:12px; }
  .totals-table td { padding:2px 0; }
  .totals-table .t-label { color:#333; }
  .totals-table .t-value { text-align:right; font-weight:bold; white-space:nowrap; }
  .totals-final td { font-size:14px; font-weight:bold; padding-top:5px; }
  .totals-final .t-value { font-size:15px; }
  .payment-line { font-size:12px; margin:2px 0; }
  .note-box { font-size:12px; margin:6px 0; }
  .note-box .note-label { font-weight:bold; }
  .footer { text-align:center; font-size:12px; margin-top:8px; line-height:1.7; }
  .footer .powered { font-size:10px; color:#888; margin-top:4px; }
</style>
</head>
<body>
<div id="receipt">
  ...
  <table class="info-table">
    <tr><td class="label">TN:</td><td class="value">${cashier}</td></tr>
    ${customerBlock}
  </table>
  <div class="dash">${dash(38)}</div>
  <table class="items-table">
    <thead><tr><th class="item-col">STT  Tên món</th><th class="price-col">Thành tiền</th></tr></thead>
    <tbody>${itemsHTML}</tbody>
  </table>
  <div class="dash">${dash(38)}</div>
  <table class="totals-table">
    <tr><td class="t-label">Thành tiền:</td><td class="t-value">${fmt(inv.originalPrice)}d</td></tr>
    ${inv.discountAmount ? `<tr class="t-discount"><td class="t-label">Giảm giá:</td><td class="t-value" style="color:#c00;">- ${fmt(inv.discountAmount)}d</td></tr>` : ""}
    ${inv.shippingFee ? `<tr><td class="t-label">Phí ship:</td><td class="t-value">${fmt(inv.shippingFee)}d</td></tr>` : ""}
    <tr class="totals-final"><td class="t-label">Tổng tiền:</td><td class="t-value">${fmt(inv.finalPrice)}d</td></tr>
  </table>
  <div class="dash">${dash(38)}</div>
  <div class="payment-line">+Thanh toán (${pmLabel}):&nbsp;&nbsp;<strong>${fmt(inv.finalPrice)}d</strong></div>
  ${inv.note ? `<div class="dash">${dash(38)}</div><div class="note-box"><span class="note-label">Ghi chú: </span>${inv.note}</div>` : ""}
  <div class="dash">${dash(38)}</div>
  <div class="footer">
    ...
    iframe.style.cssText = "position:fixed;left:-9999px;top:0;width:302px;height:1200px;border:none;opacity:0;pointer-events:none;";
    ...
    const pdf = new jsPDF({
      ...
      unit: "mm",
      format: [MM_WIDTH, imgHmm],
    });

    pdf.addImage(canvas.toDataURL("image/jpeg", 0.98), "JPEG", 0, 0, MM_WIDTH, imgHmm);
