...
<template>
  <div class="hcf-container">
    ...
    <div class="filter">
      ...
      <input v-model="search" @input="currentPage = 1" placeholder="Tìm tên loại sữa..." />
    </div>
    <table>
      <thead>
        <tr>
          <th>STT</th><th>ID</th><th>Tên loại sữa</th>
    ...
    <div class="popup-overlay" :class="{ show: showForm }" @click.self="showForm = false">
      <div class="popup">
        <div class="popup-header">
          <h3>{{ isEditing ? 'Chỉnh sửa sữa' : 'Thêm loại sữa' }}</h3>
        ...
        <div class="form-grid">
          ...
          <div class="form-group full"><label>Tên loại sữa</label><input ref="inputName" v-model="form.tenLoai" placeholder="VD: Sữa tươi, Sữa oat..." @keyup.enter="handleSubmit" /></div>
...
<!-- <script setup>
import useSua from '../JS/Sua.JS'
const { search, currentPage, PAGE_SIZE, filtered, totalPages, pageStart, pagedRows, ghostCount, fmtPrice, fmtDate, showForm, isEditing, inputName, form, openAdd, openEdit, submitForm, showConfirm, deleteTarget, openConfirm, doDelete, toastShow, toastMsg, toastType, showToast } = useSua()
...
} = useIngredients('milks', 'SUA'); // Use 'toppings' category
