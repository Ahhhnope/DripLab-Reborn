...
<template>
  <div
    class="flex w-screen h-screen overflow-hidden bg-[#FAF7F2] font-sans text-stone-700"
  >
    <aside
      class="w-64 h-full flex flex-col shrink-0 bg-white border-r border-stone-200 shadow-sm"
    >
      <div
        class="flex items-center px-6 py-6 bg-white border-b border-stone-100"
      >
        ...
        <span class="ml-3 font-bold text-[#3C2A21] text-lg tracking-tight"
          >Drip Lab</span
        >
      </div>
      ...
      <nav class="flex-1 overflow-y-auto py-6 px-4 space-y-7 custom-scrollbar">
        <div v-for="group in menuGroups" :key="group.title" class="space-y-2">
          <h3
            class="px-3 text-[10px] font-bold text-stone-400 uppercase tracking-[0.15em]"
          >
            {{ group.title }}
          </h3>
                  ...
                  <li v-for="child in item.children" :key="child.id">
                    ...
                    >
                      ...
                      <component
                        ...
                        :is="
                          openSub === child.id ? ChevronUpIcon : ChevronDownIcon
                        "
                        class="w-3 h-3"
                      />
                    </button>
                    ...
                    <transition name="expand">
                      <ul
                        v-if="child.subChildren && openSub === child.id"
                        class="ml-6 space-y-1 my-1"
                      >
      ...
      <div
        class="p-4 border-t border-stone-100 bg-[#FAF7F2]/50 flex items-center gap-3"
      >
        <div
          class="w-9 h-9 rounded-full bg-white flex items-center justify-center text-[#F5EBE0] text-xs font-bold shadow-sm"
        >
        ...
        <button
          @click="logout"
          class="p-2 text-stone-400 hover:text-red-500 rounded-lg hover:bg-red-50 transition-all"
        >
    ...
    <main class="flex-1 h-full overflow-y-auto p-8">
      <div
        class="bg-white rounded-2xl shadow-sm border border-stone-200/60 min-h-full p-6"
      >
        <RouterView v-slot="{ Component, route }">
          <KeepAlive :include="['CounterOrder']">
            <component
              :is="Component"
              :key="route.meta.keepAlive ? route.path : route.fullPath"
            />
          </KeepAlive>
        </RouterView>
      </div>
