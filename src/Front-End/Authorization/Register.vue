  ...
  <div class="lab-root">
    ...
    <div class="lab-bg">
      ...
      <svg class="tubes" viewBox="0 0 200 300" fill="none">
        <rect x="20" y="20" width="22" height="120" rx="11" stroke="#8B5E3C" stroke-width="1.5"
          fill="rgba(139,94,60,0.05)" />
        ...
        <rect x="60" y="10" width="22" height="140" rx="11" stroke="#C4956A" stroke-width="1.5"
        ...
        <rect x="100" y="30" width="22" height="100" rx="11" stroke="#6B8B5E" stroke-width="1.5"
          fill="rgba(107,139,94,0.04)" />
      ...
      <svg class="formula-bg" viewBox="0 0 300 80" fill="none">
        <text x="0" y="30" font-family="monospace" font-size="11" fill="rgba(139,94,60,0.15)">C₈H₁₀N₄O₂ + H₂O → ΔT:93°C
          → ☕</text>
        <text x="0" y="55" font-family="monospace" font-size="9" fill="rgba(139,94,60,0.1)">Caffeine + Maillard Reaction
          → Drip Lab Extract</text>
      </svg>
      <svg class="beaker-r" viewBox="0 0 80 120" fill="none">
        <path d="M20 10 L20 65 L8 105 Q6 112 14 112 L66 112 Q74 112 72 105 L60 65 L60 10 Z" stroke="#8B5E3C"
          stroke-width="1.5" fill="rgba(139,94,60,0.05)" />
    ...
    <div v-if="step === 3" class="success-card">
      <div class="success-icon">
        <svg viewBox="0 0 60 60" fill="none" width="60" height="60">
          ...
          <path d="M18 30 L26 38 L42 22" stroke="#5A9E7A" stroke-width="2" stroke-linecap="round"
            stroke-linejoin="round" class="check-draw" />
        </svg>
      </div>
    ...
    <div v-else class="lab-card" :class="{ loading: isLoading }">
      ...
      <div class="card-header">
        ...
        <div class="step-indicator">
          ...
          <div class="step-labels">
            <span class="step-lbl" :class="{ active: step >= 1, done: step > 1 }">
              <span class="step-dot">{{ step > 1 ? '✓' : '1' }}</span>
              Tài khoản
            </span>
            <span class="step-lbl" :class="{ active: step >= 2 }">
      ...
      <form v-if="step === 1" class="lab-form" @submit.prevent="handleNext">

        <div class="field-group">
          <label class="field-label"><span>Họ và tên</span></label>
          <div class="field-input-wrap">
            <svg class="field-icon" width="15" height="15" viewBox="0 0 24 24" fill="none">
              <circle cx="12" cy="8" r="4" stroke="currentColor" stroke-width="1.5" />
              <path d="M4 20c0-4 3.6-7 8-7s8 3 8 7" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" />
            </svg>
            <input v-model="name" type="text" class="field-input" placeholder="Nguyễn Văn A" autocomplete="name" />
          </div>
        </div>
        ...
        <div class="field-group">
          ...
          <div class="field-input-wrap">
            ...
            <input v-model="email" type="email" class="field-input" placeholder="researcher@driplab.com"
              autocomplete="email" />
          </div>
        </div>

        <div class="field-group">
          <label class="field-label"><span>Số điện thoại</span></label>
          <div class="field-input-wrap">
            <svg class="field-icon" width="15" height="15" viewBox="0 0 24 24" fill="none">
              <path
            ...
            <input v-model="phone" type="tel" class="field-input" placeholder="0912 345 678" autocomplete="tel" />
          </div>
        </div>

        <div class="field-group">
          <label class="field-label"><span>Mật khẩu</span></label>
          <div class="field-input-wrap">
            <svg class="field-icon" width="15" height="15" viewBox="0 0 24 24" fill="none">
              <rect x="5" y="11" width="14" height="10" rx="2" stroke="currentColor" stroke-width="1.5" />
              <path d="M8 11 V7 A4 4 0 0 1 16 7 V11" stroke="currentColor" stroke-width="1.5" />
            </svg>
            <input v-model="password" :type="showPwd ? 'text' : 'password'" class="field-input" placeholder="••••••••"
        ...
        <div class="field-group">
          ...
          <div class="field-input-wrap">
            ...
            <input v-model="confirm" :type="showCfm ? 'text' : 'password'" class="field-input"
              :class="{ mismatch: confirm && confirm !== password }" placeholder="••••••••"
              autocomplete="new-password" />
      ...
      <!-- ════ BƯỚC 2: Địa chỉ vận chuyển ════ -->
      <form v-else-if="step === 2" class="lab-form" @submit.prevent="handleRegister">

        <!-- Số nhà, tên đường -->
        <div class="field-group">
          <label class="field-label"><span>Số nhà, tên đường</span></label>
          <div class="field-input-wrap">
            <svg class="field-icon" width="15" height="15" viewBox="0 0 24 24" fill="none">
              <path d="M3 12L12 3l9 9M5 10v10h5v-6h4v6h5V10" stroke="currentColor" stroke-width="1.5"
                stroke-linecap="round" stroke-linejoin="round" />
            </svg>
            <input v-model="fullAddress" type="text" class="field-input" placeholder="123 Đường Lê Lợi" />
          </div>
        </div>

        <!-- Phường / Xã -->
        <div class="field-group">
          <label class="field-label"><span>Phường / Xã</span></label>
          <div class="field-input-wrap">
            <svg class="field-icon" width="15" height="15" viewBox="0 0 24 24" fill="none">
              <circle cx="12" cy="10" r="3" stroke="currentColor" stroke-width="1.5" />
              <path d="M12 2C7.58 2 4 5.58 4 10c0 5.5 8 12 8 12s8-6.5 8-12c0-4.42-3.58-8-8-8z" stroke="currentColor"
                stroke-width="1.5" />
            </svg>
            <input v-model="wardSearch" list="ward-list" type="text" class="field-input"
              :class="{ 'ward-selected': ward }" placeholder="Tìm phường / xã…" autocomplete="off"
              @change="onWardChange" @input="onWardInput" />
            <datalist id="ward-list">
              <option v-for="w in ALL_WARDS" :key="w" :value="w" />
            </datalist>
            <svg v-if="ward" class="ward-check" width="14" height="14" viewBox="0 0 24 24" fill="none">
              <path d="M5 13l4 4L19 7" stroke="#5A9E7A" stroke-width="2" stroke-linecap="round"
                stroke-linejoin="round" />
            </svg>
          </div>
        </div>

        <!-- Thành phố — fixed Hà Nội -->
        <div class="field-group">
          <label class="field-label"><span>Thành phố</span></label>
          <div class="field-input-wrap">
            <svg class="field-icon" width="15" height="15" viewBox="0 0 24 24" fill="none">
            ...
            <input :value="city" type="text" class="field-input city-fixed" readonly tabindex="-1" />
            <svg class="ward-check" width="14" height="14" viewBox="0 0 24 24" fill="none">
              <path d="M5 13l4 4L19 7" stroke="#5A9E7A" stroke-width="2" stroke-linecap="round"
                stroke-linejoin="round" />
            </svg>
          </div>
        ...
        <div class="btn-row">
          ...
          <button type="submit" class="submit-btn flex-1" :class="{ submitting: isLoading }">
